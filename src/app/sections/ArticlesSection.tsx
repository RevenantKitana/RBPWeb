  ...
  return (
    <section id="articles" className="py-24 px-6 bg-[#08080f]/55">
      <div className="max-w-6xl mx-auto">
        ...
        {ARTICLES.length === 0 ? (
          <div className="mt-10 rounded-3xl border border-white/10 bg-white/5 p-8 text-sm text-muted-foreground">
            {t.noArticles}
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {ARTICLES.map((article, i) => {
              const categoryLabel = article.category?.trim() ? article.category : t.noCategory;
                    ...
                    <div className="h-36 bg-muted relative overflow-hidden">
                      ...
                      <span
                        className={`absolute top-3 left-3 text-[10px] font-mono px-2 py-0.5 rounded-full ${
                          CATEGORY_COLORS[categoryLabel] ?? "bg-white/10 text-white/60"
                        }`}
                      >
                        {categoryLabel}
                      </span>
